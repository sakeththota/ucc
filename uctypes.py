...
class ArrayType(Type):
    ...
    def mangle(self):
        ...
        generation.
        """
        return 'UC_ARRAY({0})'.format(self.elem_type.mangle())
    ...
    def check_args(self, phase, position, args):
        ...
        for arg in args:
            if not is_compatible(arg.type, self.elem_type):
                error(
                    phase, position, f"type {arg.type} "
                    + "of argument is not assignable to "
                    + f"array element of type {self.elem_type}")
    ...
    def lookup_field(self, phase, position, name, global_env):
        ...
        if name == 'length':
            return global_env.lookup_type(6, position, 'int')
        error(6, position, f"{name} is not a valid field of {self.name}")
        return global_env.lookup_type(6, position, 'int')
...
class PrimitiveType(Type):
    ...
    def mangle(self):
        ...
        generation.
        """
        return 'UC_PRIMITIVE({0})'.format(self.name)
...
class UserType(Type):
    ...
    def check_args(self, phase, position, args):
        ...
        if len(args) != 0 and len(args) != len(self.fields):
            error(phase, position,
                  f"user-defined type {self.name} expected 0 "
                  + f"or {len(self.fields)} "
                  + f"argument(s), but got {len(args)}")
        else:
            for arg, field in zip(args, self.fields):
                if not is_compatible(arg.type, field.vartype.type):
                    error(phase, position,
                          f"type {arg.type} of argument is "
                          + "not compatible with field of "
                          + f"type {field.vartype.type}")
    ...
    def lookup_field(self, phase, position, name, global_env):
        ...
        for field in self.fields:
            if field.name.raw == name:
                return field.vartype.type
        error(6, position,
              f"{name} field not found in user-defined type: {self.name}")
        return global_env.lookup_type(phase, position, 'int')
    ...
    def mangle(self):
        ...
        generation.
        """
        return 'UC_REFERENCE({0})'.format(self.name)
